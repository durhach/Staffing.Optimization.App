...
base.theme <- readRDS("C:/Users/mhhccd/Documents/Tools/R/Shared Drive R Code/Chart_SetUp/base.theme.RDS")
chart.list <- readRDS("C:/Users/mhhccd/Documents/Tools/R/Shared Drive R Code/Chart_SetUp/chart.list.RDS")
Wday.factors <- readRDS("C:/Users/mhhccd/Documents/Tools/R/Shared Drive R Code/Chart_SetUp/Wday.factors.RDS")
...
options(shiny.maxRequestSize=30*1024^2) # increase max csv upload size to 30MB
...
raw.stats <- boxplot(raw3$LOS)$stats
row.names(raw.stats) <- c("Minimum","1st Quartile","Median", "Third Quartile","Maximum")
df3.list[[4]] <- raw.stats
names(df3.list)[[4]] <- "bp.stats.all"

filtered.stats <- boxplot(dff$LOS)$stats
row.names(filtered.stats) <- c("Minimum","1st Quartile","Median", "Third Quartile","Maximum")
df3.list[[5]] <- filtered.stats
names(df3.list)[[5]] <- "bp.stats.filtered"
...
inputparameters <- eventReactive(c(df(),input$ts2,input$outliers,input$inDateRange,input$custom.percentile,
   ...
   StartDTS =                 input$inDateRange[[1]], # Minimum selected StartDTS
   EndDTS =                   input$inDateRange[[2]], # Maximum selected EndDTS
 ...
 output$optim.downloadData <- downloadHandler(
    ...
    content = function(file){
      ...
      addDataFrame(df()[[3]], sheet=sheet.1, startRow=1, # Raw Data
      ...
      setColumnWidth(sheet.1,colIndex=c(6:11),colWidth=30)
      addDataFrame(df()[[5]], sheet=sheet.1, startRow=9, # Arrival Frequency Table 
                   startColumn=10,row.names=FALSE)
      setColumnWidth(sheet.1,colIndex=c(10:11),colWidth=30)
      addDataFrame(df3()[[4]], sheet=sheet.1, startRow=12, # Boxplot Summary Data
                   startColumn=8,row.names=TRUE)
      setColumnWidth(sheet.1,colIndex=c(10:11),colWidth=30)
      addDataFrame(df3()[[5]], sheet=sheet.1, startRow=39, # Boxplot Summary Data
                   startColumn=8,row.names=TRUE)
      setColumnWidth(sheet.1,colIndex=c(10:11),colWidth=30)
      ...
      addPicture(file = "C:/Users/mhhccd/Dropbox/Work.Synched/Staffing.Optimization.App/arrival.curve.jpeg", sheet = sheet.1, scale = 1,startRow = 12, startColumn = 13)
      ...
      addPicture(file = "C:/Users/mhhccd/Dropbox/Work.Synched/Staffing.Optimization.App/bp.all.data.jpeg", sheet = sheet.1, scale = 0.75,startRow = 12, startColumn = 6)
      ...
      addPicture(file = "C:/Users/mhhccd/Dropbox/Work.Synched/Staffing.Optimization.App/bp.filtered.data.jpeg", sheet = sheet.1, scale = 0.75,startRow = 39, startColumn = 6)
      ...
      addDataFrame(inputparameters(), sheet=sheet.2, startRow=1, 
                   startColumn=1,row.names=FALSE)
      setColumnWidth(sheet.2,colIndex=c(1:45),colWidth=30)  
      ...
      addPicture(file = "C:/Users/mhhccd/Dropbox/Work.Synched/Staffing.Optimization.App/shift.schedule.jpeg", sheet = sheet.4, scale = 1,startRow = nrow(optim_graphs()[[6]])+2, startColumn = 1)
      ...
      # add sheet 6 Start Hour data & plot
